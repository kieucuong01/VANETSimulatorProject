...
public final class Renderer{
	...
	private Renderer(){
		try{
			...
			tmpImage = ImageIO.read(url);
			vehicleImg_ = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(tmpImage.getWidth(), tmpImage.getHeight(), 3);
			vehicleImg_.getGraphics().drawImage(tmpImage, 0, 0, null);
			
			url = ClassLoader.getSystemResource("vanetsim/images/TA.png"); 
			tmpImage = ImageIO.read(url);
			TAImg_ = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration().createCompatibleImage(tmpImage.getWidth(), tmpImage.getHeight(), 3);
						...
						for(j = regionMinY_; j <= regionMaxY_; ++j){
							...
							for(k = 0; k < size; ++k){
								...
								if((isShowVehicles() || vehicle.isActive()) && vehicle.getX() >= mapMinX_ && vehicle.getX() <= mapMaxX_ && vehicle.getY() >= mapMinY_ && vehicle.getY() <= mapMaxY_){		//only paint when necessary and within paint area
									...
									g2d.drawImage(vehicleImg_, vehicle.getX()-VEHICLE_SIZE/2,  vehicle.getY()-VEHICLE_SIZE/2, VEHICLE_SIZE, VEHICLE_SIZE, null);
								}
							}
						}
					}
				...
				for(i = regionMinX_; i <= regionMaxX_; ++i){
					for(j = regionMinY_; j <= regionMaxY_; ++j){
						...
						for(int l = 0; l < rsus.length; ++l){	
							...
							//g2d.drawImage(RSUImg_, rsus[l].getX() - 250, rsus[l].getX() - 200, 500, 500, null);
							g2d.drawImage(RSUImg_, rsus[l].getX()-VEHICLE_SIZE/2,  rsus[l].getY()-VEHICLE_SIZE/2, VEHICLE_SIZE, VEHICLE_SIZE, null);
						}
					}
				}
			}
			...
			if(true){
				...
				if(tempARSUList.length > 0){
				    for(int l = 0; l < tempARSUList.length;l++) {
						...
						g2d.drawImage(TAImg_, tempARSUList[l].getX() - 2500, tempARSUList[l].getY() - 2500, 5000, 5000, null);
				      }
				}
			}
