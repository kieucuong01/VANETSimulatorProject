...
public final class Renderer {
	...
	private static final int VEHICLE_SIZE = 10000;

	/**
	 * A global formatter to get locale-specific grouping separators from numeric
	 * values
	 */
	private static final DecimalFormat FORMATTER = new DecimalFormat(",###"); //$NON-NLS-1$

	/**
	 * A reference to the singleton instance of the {@link vanetsim.map.Map} because
	 * we need this quite often and don't want to rely on compiler inlining.
	 */
	...
	/**
	 * A cached <code>BasicStroke</code> used as a (black) background for streets
	 * with 2 lanes total.
	 */
	private final BasicStroke lineBackground_ = new BasicStroke((Map.LANE_WIDTH + 45) * 2, BasicStroke.CAP_ROUND,
			BasicStroke.JOIN_MITER);

	/**
	 * A cached <code>BasicStroke</code> used for painting streets with 2 lanes
	 * total.
	 */
	private final BasicStroke lineMain_ = new BasicStroke(Map.LANE_WIDTH * 2, BasicStroke.CAP_ROUND,
			BasicStroke.JOIN_MITER);

	/**
	 * The <code>AffineTransform</code> which handles zooming and panning
	...
	/**
	 * Used to display how long the relative time in milliseconds since the
	 * simulation started.
	 */
	private int timePassed_ = 0;

	/**
	 * A boolean indicating if a new full render (=rendering of static objects) is
	 * needed.
	 */
	private boolean scheduleFullRender_ = false;

	/**
	 * <code>true</code> if the current painting was forced by the simulation
	 * master, else <code>false</code> (by AWT/Swing or so).
	 */
	...
	/**
	 * If the current rendered view displays over the borders of the map. Used to
	 * detect when efficient panning can NOT be used.
	 */
	private boolean currentOverdrawn_ = true;

	/**
	 * If the last rendered view displays over the borders of the map. Used to
	 * detect when efficient panning can NOT be used.
	 */
	...
	/**
	 * The amount of pans in x direction (negative if moved down) since the last
	 * paint of the static objects.
	 */
	private int panCountX_ = 0;

	/**
	 * The amount of pans in y direction (negative if moved down) since the last
	 * paint of the static objects.
	 */
	private int panCountY_ = 0;

	/**
	 * The x-axis index of the leftmost region which is currently considered to be
	 * rendered.
	 */
	private int regionMinX_ = 0;

	/**
	 * The y-axis index of the leftmost region which is currently considered to be
	 * rendered.
	 */
	private int regionMinY_ = 0;

	/**
	 * The x-axis index of the rightmost region which is currently considered to be
	 * rendered.
	 */
	private int regionMaxX_ = 0;

	/**
	 * The y-axis index of the rightmost region which is currently considered to be
	 * rendered.
	 */
	private int regionMaxY_ = 0;

	/**
	 * A temporary <code>AffineTransform</code>. Prevents some unnecessary garbage
	 * collection.
	 */
	private AffineTransform tmpAffine_ = new AffineTransform();

	/**
	 * The maximum translation in x direction we got from a <code>Graphics2D</code>
	 * object (used because this value sometimes is wrong when using substance
	 * themes).
	 */
	private double maxTranslationX_ = 0;

	/**
	 * The maximum translation in y direction we got from a <code>Graphics2D</code>
	 * object (used because this value sometimes is wrong when using substance
	 * themes).
	 */
	...
	/**
	 * A <code>CyclicBarrier</code> to signal the SimulationMaster that we are ready
	 * with rendering the dynamic objects.
	 */
	...
	/**
	 * A scaled instance of the <code>blockingImage_</code>. Is updated on every
	 * zoom change.
	 */
	private BufferedImage scaledBlockingImage_;

	/**
	 * A scaled instance of the <code>slipperyImage_</code>. Is updated on every
	 * zoom change.
	 */
	private BufferedImage scaledSlipperyImage_;

	private BufferedImage vehicleImg_;
	...
	/**
	 * arraylist to show if a beacon was guessed correctly after mix. Is not used
	 * while simulation is running, so performance doesn't have to be that good
	 */
	private ArrayList<String> locationInformationMix_ = null;

	/**
	 * arraylist to show if a beacon was guessed correctly after silent period. Is
	 * not used while simulation is running, so performance doesn't have to be that
	 * good
	 */
	...
	/**
	 * flag to activate the debug mode. In this mode more information is shown to
	 * the user
	 */
	private boolean debugMode = false;

	/**
	 * flag to enable schools, police, ... on map (this are not event spots, but can
	 * be imported)
	 */
	...
	private Renderer() {
		try {
			// We're not directly using the BufferedImage returned by ImageIO as this might
			// not be suitable for the graphics environment and thus slow. The small
			// overhead here does not hurt
			...
			blockingImage_ = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
					.getDefaultConfiguration().createCompatibleImage(tmpImage.getWidth(), tmpImage.getHeight(), 3);
			...
			tmpImage = ImageIO.read(url);
			slipperyImage_ = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
					.getDefaultConfiguration().createCompatibleImage(tmpImage.getWidth(), tmpImage.getHeight(), 3);
			slipperyImage_.getGraphics().drawImage(tmpImage, 0, 0, null);

			url = ClassLoader.getSystemResource("vanetsim/images/vehicle.png");
			tmpImage = ImageIO.read(url);
			vehicleImg_ = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
					.getDefaultConfiguration().createCompatibleImage(tmpImage.getWidth(), tmpImage.getHeight(), 3);
			vehicleImg_.getGraphics().drawImage(tmpImage, 0, 0, null);

			url = ClassLoader.getSystemResource("vanetsim/images/TA.png");
			tmpImage = ImageIO.read(url);
			TAImg_ = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
					.getDefaultConfiguration().createCompatibleImage(tmpImage.getWidth(), tmpImage.getHeight(), 3);
			TAImg_.getGraphics().drawImage(tmpImage, 0, 0, null);

			url = ClassLoader.getSystemResource("vanetsim/images/RSU.png");
			tmpImage = ImageIO.read(url);
			RSUImg_ = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
					.getDefaultConfiguration().createCompatibleImage(tmpImage.getWidth(), tmpImage.getHeight(), 3);
			RSUImg_.getGraphics().drawImage(tmpImage, 0, 0, null);

			url = ClassLoader.getSystemResource("vanetsim/images/speech_buble.png");
			tmpImage = ImageIO.read(url);
			speechBuble_ = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
					.getDefaultConfiguration().createCompatibleImage(tmpImage.getWidth(), tmpImage.getHeight(), 3);
			...
			blockingImage_ = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
					.getDefaultConfiguration().createCompatibleImage(1, 1, 3);
			slipperyImage_ = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice()
					.getDefaultConfiguration().createCompatibleImage(1, 1, 3);
		}
	}
	 ...
	 * 
	 * @param g2d
	 *            the <code>Graphics2D</code> object on which rendering takes place
	 * @throws IOException
	 */
	public void drawMovingObjects(Graphics2D g2d) throws IOException {
		...
		if (regions != null && map_.getReadyState() && (!simulationRunning_ || doPaintInitializedBySimulation_)) {
			...
			// A small fix because the substance theme engine sometimes causes unwanted
			// shifts
			...
			if (showBeaconMonitorZone_) {
				...
				g2d.drawRect(beaconMonitorMinX_, beaconMonitorMinY_, beaconMonitorMaxX_ - beaconMonitorMinX_,
						beaconMonitorMaxY_ - beaconMonitorMinY_);
			}
			...
			if (markedStreet_ != null) {
				...
				g2d.setStroke(new BasicStroke(Map.LANE_WIDTH * totalLanes + 45, BasicStroke.CAP_ROUND,
						BasicStroke.JOIN_MITER));
				g2d.setPaint(Color.BLACK);
				g2d.drawLine(x0, y0, x1, y1);

				// paint arrows to indicate direction. See
				// http://lifshitz.ucdavis.edu/~dmartin/teach_java/slope/arrows.html (link last
				// visited: 04.09.2008)
				int deltaX = x1 - x0;
				int deltaY = y1 - y0;
				double frac = 0.2;
				if (markedStreet_.getLength() < 1000 && markedStreet_.isOneway())
					frac = 0.9; // paint short streets which are oneway with longer arrow. There's no solution
								// implemented for short twoway-streets...

				g2d.drawLine(x0 + (int) ((1 - frac) * deltaX + frac * deltaY),
				...
				if (!markedStreet_.isOneway()) {
					deltaX = -deltaX;
					deltaY = -deltaY;
					g2d.drawLine(x1 + (int) ((1 - frac) * deltaX + frac * deltaY),
							y1 + (int) ((1 - frac) * deltaY - frac * deltaX), x0, y0);
					g2d.drawLine(x1 + (int) ((1 - frac) * deltaX - frac * deltaY),
							y1 + (int) ((1 - frac) * deltaY + frac * deltaX), x0, y0);
					deltaX = -deltaX;
					deltaY = -deltaY;
				...
				g2d.setStroke(
						new BasicStroke(Map.LANE_WIDTH * totalLanes, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER));
			...
			if (showAllBlockings_)
				blockings = EventList.getInstance().getAllBlockingsArrayList();
			else
				blockings = EventList.getInstance().getCurrentBlockingsArrayList();
			size = blockings.size();
			if (size > 0) {
				k = scaledBlockingImage_.getHeight() / 2;
				int l = scaledBlockingImage_.getHeight() / 2;
				for (i = 0; i < size; ++i) {
					blockingImageTransformSource_.setLocation(blockings.get(i).getX() - k / zoom_,
							blockings.get(i).getY() - l / zoom_);
					...
					if (blockings.get(i).getPenaltyType_().equals("HUANG_RHCN")) {
						g2d.drawImage(scaledSlipperyImage_, (int) Math.round(blockingImageTransformDestination_.getX()),
								(int) Math.round(blockingImageTransformDestination_.getY()), null, drawArea_);
					} else {
						g2d.drawImage(scaledBlockingImage_, (int) Math.round(blockingImageTransformDestination_.getX()),
								(int) Math.round(blockingImageTransformDestination_.getY()), null, drawArea_);
					}
				}
			}
					...
					for (k = 0; k < streets.length; ++k) {
						...
						if (street.getEndNodeTrafficLightState() != -1) {
							if (markedJunction_ != null && markedJunction_.getNode().equals(street.getEndNode())) {
								...
								g2d.fillOval(street.getTrafficLightEndX_() - (Map.LANE_WIDTH + 45),
										street.getTrafficLightEndY_() - (Map.LANE_WIDTH + 45),
										(Map.LANE_WIDTH + 45) * 2, (Map.LANE_WIDTH + 45) * 2);
							}
							if (street.getEndNodeTrafficLightState() == 0)
								g2d.setPaint(Color.green);
							else if (street.getEndNodeTrafficLightState() == 1
									|| street.getEndNodeTrafficLightState() == 7)
								g2d.setPaint(Color.yellow);
							else
								g2d.setPaint(Color.red);

							g2d.fillOval(street.getTrafficLightEndX_() - (Map.LANE_WIDTH),
									street.getTrafficLightEndY_() - (Map.LANE_WIDTH), (Map.LANE_WIDTH) * 2,
									(Map.LANE_WIDTH) * 2);
						}
						if (street.getStartNodeTrafficLightState() != -1) {
							if (markedJunction_ != null && markedJunction_.getNode().equals(street.getStartNode())) {
								g2d.setPaint(Color.orange);
								g2d.fillOval(street.getTrafficLightStartX_() - (Map.LANE_WIDTH + 45),
										street.getTrafficLightStartY_() - (Map.LANE_WIDTH + 45),
										(Map.LANE_WIDTH + 45) * 2, (Map.LANE_WIDTH + 45) * 2);
							}
							if (street.getStartNodeTrafficLightState() == 0)
								g2d.setPaint(Color.green);
							else if (street.getStartNodeTrafficLightState() == 1
									|| street.getStartNodeTrafficLightState() == 7)
								g2d.setPaint(Color.yellow);
							else
								g2d.setPaint(Color.red);

							g2d.fillOval(street.getTrafficLightStartX_() - (Map.LANE_WIDTH),
									street.getTrafficLightStartY_() - (Map.LANE_WIDTH), (Map.LANE_WIDTH) * 2,
									(Map.LANE_WIDTH) * 2);
						}
					}
				}
			...
			if (true) {
					...
					for (i = regionMinX_; i <= regionMaxX_; ++i) {
						for (j = regionMinY_; j <= regionMaxY_; ++j) {
							...
							for (k = 0; k < size; ++k) {
								...
								if ((isShowVehicles() || vehicle.isActive()) && vehicle.getX() >= mapMinX_
										&& vehicle.getX() <= mapMaxX_ && vehicle.getY() >= mapMinY_
										&& vehicle.getY() <= mapMaxY_) { // only paint when necessary and within paint
																			// area
									if (highlightCommunication_) {
										if (vehicle.isWiFiEnabled()
												&& (!vehicle.isInMixZone() || Vehicle.getMixZonesFallbackEnabled())) {
											g2d.setPaint(Color.blue);
											if (vehicle != markedVehicle_)
												g2d.drawOval(vehicle.getX() - vehicle.getMaxCommDistance(),
														vehicle.getY() - vehicle.getMaxCommDistance(),
														vehicle.getMaxCommDistance() * 2,
														vehicle.getMaxCommDistance() * 2);
										} else
											g2d.setPaint(Color.black);
									}
									g2d.drawImage(vehicleImg_, vehicle.getX() - VEHICLE_SIZE / 2,
											vehicle.getY() - VEHICLE_SIZE / 2, VEHICLE_SIZE, VEHICLE_SIZE, null);
								}
							}
						}
					}
					...
					try {
						for (i = regionMinX_; i <= regionMaxX_; ++i) {
							for (j = regionMinY_; j <= regionMaxY_; ++j) {
								...
								for (k = 0; k < size; ++k) {
									vehicle = vehicles[k];
									if (vehicle.isActive() && vehicle.getX() >= mapMinX_ && vehicle.getX() <= mapMaxX_
											&& vehicle.getY() >= mapMinY_ && vehicle.getY() <= mapMaxY_) { // only paint
																											// when
																											// necessary
																											// and
																											// within
																											// paint
																											// area
										g2d.drawImage(speechBuble_, vehicle.getX() - VEHICLE_SIZE / 2,
												vehicle.getY() - 20000, 20000, 20000, null);
										...
										if (scenario_ == 12) {
											if (destinationPoint.getX() == 87687 && destinationPoint.getY() == 29878) {
												g2d.drawString("Sending...", vehicle.getX() - 2000,
														vehicle.getY() - 15000);
												g2d.drawString("SN1 = 1111", vehicle.getX() - 2000,
														vehicle.getY() - 12500);
												g2d.drawString("ID = 93K1-07526", vehicle.getX() - 2000,
														vehicle.getY() - 10000);

											} else {
												g2d.drawString("Sending...", vehicle.getX() - 2000,
														vehicle.getY() - 15000);
												g2d.drawString("SN1 = 222", vehicle.getX() - 2000,
														vehicle.getY() - 12500);
												g2d.drawString("ID = 63K1-01236", vehicle.getX() - 2000,
														vehicle.getY() - 10000);
											}
										}
										...
										if (scenario_ == 13) {
											if (destinationPoint.getX() == 87687 && destinationPoint.getY() == 29878) {
												g2d.drawString("Psudonym : \n", vehicle.getX() - 2000,
														vehicle.getY() - 15000);
												g2d.drawString(vehicle1Pseudonum, vehicle.getX() - 2000,
														vehicle.getY() - 12500);
											} else {
												g2d.drawString("Psudonym : \n", vehicle.getX() - 2000,
														vehicle.getY() - 15000);
												g2d.drawString(vehicle2Pseudonum, vehicle.getX() - 2000,
														vehicle.getY() - 12500);
											}
										}
										...
										if (scenario_ == 2) {
											if (didSendSuccess_) {
												if (destinationPoint.getX() == 87687
														&& destinationPoint.getY() == 29878) {
									...
									if (vehicle.isActive() && vehicle.getX() >= mapMinX_ && vehicle.getX() <= mapMaxX_
										...
										if (tmp != null) {
											for (int p = 0; p < tmp.getSize(); p++) {
												if (tmp.getPenaltyType_()[p].equals("EVA_EMERGENCY_ID"))
													g2d.setPaint(Color.red);
												else if (tmp.getPenaltyType_()[p].equals("HUANG_PCN"))
													g2d.setPaint(Color.black);
												else if (tmp.getPenaltyType_()[p].equals("PCN_FORWARD"))
													g2d.setPaint(Color.gray);
												else if (tmp.getPenaltyType_()[p].equals("HUANG_RHCN"))
													g2d.setPaint(Color.green);
												else if (tmp.getPenaltyType_()[p].equals("HUANG_EVA_FORWARD"))
													g2d.setPaint(Color.pink);
												else if (tmp.getPenaltyType_()[p].equals("HUANG_EEBL"))
													g2d.setPaint(Color.orange);

												if (tmp.getPenaltySourceVehicle_() != null
														&& tmp.getPenaltySourceVehicle_()[p] != null)
													g2d.drawLine(vehicle.getX(), vehicle.getY(),
															tmp.getPenaltySourceVehicle_()[p].getX(),
															tmp.getPenaltySourceVehicle_()[p].getY());
											}
										}
				...
				if (markedVehicle_ != null) {
					g2d.setPaint(Color.RED);
					g2d.fillOval(markedVehicle_.getX() - 100 / 2 + 35, markedVehicle_.getY() - 100 / 2 + 35, 100 - 70,
							100 - 70);
					if (markedVehicle_.isWiFiEnabled()
							&& (!markedVehicle_.isInMixZone() || Vehicle.getMixZonesFallbackEnabled()))
						g2d.drawOval(markedVehicle_.getX() - markedVehicle_.getMaxCommDistance(),
								markedVehicle_.getY() - markedVehicle_.getMaxCommDistance(),
								markedVehicle_.getMaxCommDistance() * 2, markedVehicle_.getMaxCommDistance() * 2);
					WayPoint nextDestination = markedVehicle_.getDestinations().peekFirst();
					if (nextDestination != null) {
						g2d.drawLine(markedVehicle_.getX(), markedVehicle_.getY(), nextDestination.getX(),
						...
						if (routestreets.length > 1) {
							...
							if (markedVehicle_.getCurDirection())
								g2d.drawLine(markedVehicle_.getX(), markedVehicle_.getY(),
										markedVehicle_.getCurStreet().getEndNode().getX(),
										markedVehicle_.getCurStreet().getEndNode().getY());
							else
								g2d.drawLine(markedVehicle_.getX(), markedVehicle_.getY(),
										markedVehicle_.getCurStreet().getStartNode().getX(),
										markedVehicle_.getCurStreet().getStartNode().getY());
							for (i = markedVehicle_.getRoutePosition() + 1; i < routestreets.length - 1; ++i) {
								g2d.drawLine(routestreets[i].getStartNode().getX(),
										routestreets[i].getStartNode().getY(), routestreets[i].getEndNode().getX(),
										routestreets[i].getEndNode().getY());
							}
							if (!markedVehicle_.getRouteDirections()[routestreets.length - 1]) {
								g2d.drawLine(nextDestination.getX(), nextDestination.getY(),
										routestreets[routestreets.length - 1].getEndNode().getX(),
										routestreets[routestreets.length - 1].getEndNode().getY());
							} else {
								g2d.drawLine(nextDestination.getX(), nextDestination.getY(),
										routestreets[routestreets.length - 1].getStartNode().getX(),
										routestreets[routestreets.length - 1].getStartNode().getY());
							}
						}
					}

					// added to display more than 2 Waypoints for one vehicle (only used in vehicle
					// edit mode, so no need to improve the performance)
					if (isShowVehicles()) {
						...
						for (WayPoint destination : tmpDestinations) {
							if (oldDestination != null) {
								g2d.setPaint(Color.blue);
								g2d.fillOval(oldDestination.getX() - VEHICLE_SIZE, oldDestination.getY() - VEHICLE_SIZE,
										VEHICLE_SIZE * 2, VEHICLE_SIZE * 2);
								g2d.setPaint(Color.red);
								g2d.drawLine(oldDestination.getX(), oldDestination.getY(), destination.getX(),
										destination.getY());
							}

							g2d.setPaint(Color.red);
							g2d.fillOval(destination.getX() - VEHICLE_SIZE, destination.getY() - VEHICLE_SIZE,
									VEHICLE_SIZE * 2, VEHICLE_SIZE * 2);
				...
				if (showAttackers_ || simulationRunning_) {
					...
					if (attackerVehicle_ != null) {
						...
						g2d.fillOval(attackerVehicle_.getX() - VEHICLE_SIZE / 2 + 35,
								attackerVehicle_.getY() - VEHICLE_SIZE / 2 + 35, VEHICLE_SIZE - 70, VEHICLE_SIZE - 70);
					}
					// draw attacked vehicle
					if (attackedVehicle_ != null) {
						g2d.setPaint(Color.GREEN);
						g2d.fillOval(attackedVehicle_.getX() - VEHICLE_SIZE / 2 + 35,
								attackedVehicle_.getY() - VEHICLE_SIZE / 2 + 35, VEHICLE_SIZE - 70, VEHICLE_SIZE - 70);
					}
				}
			}

			// draw all nodes coloured to aid editing (only when editing streets and near
			// enough)
				...
				for (i = regionMinX_; i <= regionMaxX_; ++i) {
					for (j = regionMinY_; j <= regionMaxY_; ++j) {
						...
						for (k = 0; k < nodes.length; ++k) {
							node = nodes[k];
							g2d.setPaint(Color.black);
							g2d.fillOval(node.getX() - (Map.LANE_WIDTH + 45), node.getY() - (Map.LANE_WIDTH + 45),
									(Map.LANE_WIDTH + 45) * 2, (Map.LANE_WIDTH + 45) * 2);
							g2d.setPaint(Color.pink);
							// marked junction
							if (markedJunction_ != null && markedJunction_.getNode().equals(node))
								g2d.setPaint(Color.red);
							g2d.fillOval(node.getX() - Map.LANE_WIDTH, node.getY() - Map.LANE_WIDTH, Map.LANE_WIDTH * 2,
									Map.LANE_WIDTH * 2);
						}
					}
				}
	 ...
	 * 
	 * @param image
	 *            the <code>BufferedImage</code> on which rendering should be done
	 */
	public void drawScale(BufferedImage image) {
		...
		try {
			...
			// now find an "appropriate" length which is smaller than 70px and an even
			// multiple of a base distance (for example 7km, 300m, 20m instead of 7.2km,
			// 312m and 24.2m)
			if (len < 1) {
	...
	/**
	 * This function renders all non-moving objects on a supplied image. All streets
	 * and other static objects are rendered into the cached Image in this function.
	 ...
	 * <br>
	 * 
	 * Note: The main performance factor here are the calls to <code>draw()</code>
	 * or <code>drawLine()</code>. This is a factor determined by the underlying
	 * graphics subsytem and there's not much space for lot of improvement.
	 * Multithreading approaches are not possible here: The graphics subsystem is
	 * just singlethreaded (blocks concurrent draw-Calls even if they are done on
	 * different BufferedImages) and so there's no performance benefit.<br>
	 * Optimizing anything concerned with iterating through the regions is not worth
	 * it because the cpu time for this is barely measurable after using
	 * <code>ArrayLists</code> instead of <code>Iterators</code> (iterators create
	 * lots of object creations and calls)<br>
	 * 
	 * @param image
	 *            the <code>BufferedImage</code> on which rendering should be done
	 */
	public synchronized void drawStaticObjects(BufferedImage image) {
		...
		if (regions != null && map_.getReadyState()) {
			...
			// try to reuse image elements if possible (only possible when panning); this
			// reduces rendering time by up to 50%!
			if (!lastOverdrawn_ && (((panCountX_ == 1 || panCountX_ == -1) && panCountY_ == 0)
					|| ((panCountY_ == 1 || panCountY_ == -1) && panCountX_ == 0))) { // only ONE change in ONE
																						// direction has happened since
																						// last render
				// draw old image onto new one with an offset in x or y direction
				g2d.drawImage(image, (int) Math.round(panCountX_ * drawWidth_ / 2),
						(int) Math.round(panCountY_ * drawHeight_ / 2), null, drawArea_);
			...
			// Check if we're near enough to use antialiasing (quite costly if there are too
			// many streets)
			...
			Street[] streets;
			TreeMap<Integer, Path2D.Float> layers = new TreeMap<Integer, Path2D.Float>(); // a sorted map so that we
																							// always get the same
																							// result (basically sorted
																							// by color)
				...
				for (i = regionMinX_; i <= regionMaxX_; ++i) {
					for (j = regionMinY_; j <= regionMaxY_; ++j) {
						...
						for (int l = 0; l < nodes.length; ++l) {
							g2d.setPaint(Color.LIGHT_GRAY);
							g2d.fillOval(nodes[l].getX() - nodes[l].getMixZoneRadius(),
									nodes[l].getY() - nodes[l].getMixZoneRadius(), nodes[l].getMixZoneRadius() * 2,
									nodes[l].getMixZoneRadius() * 2);
							g2d.setPaint(Color.RED);
							if (nodes[l].getEncryptedRSU_() != null) {
								g2d.fillOval(nodes[l].getEncryptedRSU_().getX() - 1500,
										nodes[l].getEncryptedRSU_().getY() - 1500, 3000, 3000);
							}
						}
					}
				}
			...
			if (locationInformationMix_ != null) {
				...
				for (String location : locationInformationMix_) {
					...
					if (Float.parseFloat(data[4]) != 0)
						...
						size = 0;
					g2d.setPaint(new Color((int) (Float.parseFloat(data[2]) * 200) + 55,
							(int) (Float.parseFloat(data[3]) * 200) + 55, 0));
					g2d.fillOval(Integer.parseInt(data[0]) - size, Integer.parseInt(data[1]) - size, size * 2,
							size * 2);
				}
			}

			// Create and draw a black background so that streets have black boundaries.
			// Only makes sense when antialias is on because otherwise it looks ugly and is
			// really slow(!)
			if (antialias) {
				for (i = savedRegionMinX; i <= savedRegionMaxX; ++i) {
					for (j = savedRegionMinY; j <= savedRegionMaxY; ++j) {
						...
						for (k = 0; k < streets.length; ++k) {
							...
							if (streetRegion == regions[i][j] || streetRegion.getX() < savedRegionMinX
									|| streetRegion.getX() > savedRegionMaxX || streetRegion.getY() < savedRegionMinY
									|| streetRegion.getY() > savedRegionMaxY) { // only paint it if necessary (to
																				// prevent painting it multiple times)
								if (street.isOneway())
				...
				while (iterator.hasNext()) {
					...
					if (key == 2)
						...
						g2d.setStroke(new BasicStroke(Map.LANE_WIDTH * key + 90, BasicStroke.CAP_ROUND,
								BasicStroke.JOIN_MITER));
					// FIXME change for black white mode
					g2d.draw(layers.get(key));
				}
			}
			// now create the layers. This second iteration step (only second one if
			// antialiasing is on) is actually faster than mixing it into the first one!)
			layers.clear(); // to store the layers of the map separated by color
			lastEntry = 1;
			boolean detailedView;
			if (zoom_ > 0.0001)
				detailedView = true; // only show fast speed streets if zoom is very far away
			else
				detailedView = false;
			for (i = savedRegionMinX; i <= savedRegionMaxX; ++i) {
				for (j = savedRegionMinY; j <= savedRegionMaxY; ++j) {
					...
					for (k = 0; k < streets.length; ++k) {
						...
						if (streetRegion == regions[i][j] || streetRegion.getX() < savedRegionMinX
								|| streetRegion.getX() > savedRegionMaxX || streetRegion.getY() < savedRegionMinY
								|| streetRegion.getY() > savedRegionMaxY) { // only paint it if necessary (to prevent
																			// painting it multiple times)
							if (detailedView || street.getSpeed() > 2000) {
								...
								if (antialias) {
									...
									if (street.isOneway())
										...
										totalLanes = street.getDisplayColor().getRGB() * 100
												- (2 * street.getLanesCount());
								} else
			...
			while (coloriterator.hasNext()) {
				...
				if (antialias) {
					...
					if (lanes == 2)
						...
						g2d.setStroke(
								new BasicStroke(Map.LANE_WIDTH * lanes, BasicStroke.CAP_ROUND, BasicStroke.JOIN_MITER));
				} else {
					g2d.setStroke(lineMain_);
					drawColor = new Color(Math.max((int) (((key >> 16) & 0xFF) * 0.8), 0),
							Math.max((int) (((key >> 8) & 0xFF) * 0.8), 0),
							Math.max((int) (((key >> 0) & 0xFF) * 0.8), 0)); // simulate colors like if they were with
																				// antialias (darker)!
				}
				// FIXME change for black white mode
			...
			// If the zoom is near enough, do a correction to display bridges more
			// accurately (though not perfect as intersection calculation
							...
							if (streetRegion == regions[i][j] || streetRegion.getX() < savedRegionMinX
								...
								if (street.getBridgePaintLines() != null || street.getBridgePaintPolygons() != null) {
									...
									if (street.getLength() > correction * 2) {
										MapHelper.calculateResizedLine(start, end, correction, correctStart,
												correctEnd);
										// paint the now shorter line completely
										g2d.setStroke(new BasicStroke(Map.LANE_WIDTH * totalLanes + 90,
												BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER));
										// FIXME change for black white mode
										// g2d.setColor(Color.black);
										g2d.setColor(Color.white);
										g2d.drawLine(start[0], start[1], end[0], end[1]);
										g2d.setStroke(new BasicStroke(Map.LANE_WIDTH * totalLanes,
				...
				for (i = regionMinX_; i <= regionMaxX_; ++i) {
					for (j = regionMinY_; j <= regionMaxY_; ++j) {
						...
						for (int l = 0; l < rsus.length; ++l) {
							if (highlightCommunication_)
								g2d.drawOval(rsus[l].getX() - rsus[l].getWifiRadius(),
										rsus[l].getY() - rsus[l].getWifiRadius(), rsus[l].getWifiRadius() * 2,
										rsus[l].getWifiRadius() * 2);
							// g2d.fillOval(rsus[l].getX() - 250, rsus[l].getY() - 250,500,500);
							// g2d.drawImage(RSUImg_, rsus[l].getX() - 250, rsus[l].getX() - 200, 500, 500,
							// null);
							g2d.drawImage(RSUImg_, rsus[l].getX() - VEHICLE_SIZE / 2, rsus[l].getY() - VEHICLE_SIZE / 2,
					...
					for (j = regionMinY_; j <= regionMaxY_; ++j) {
						...
						if (xxx != null) {
							...
							for (k = 0; k < xxx.size(); k++) {
								g2d.drawOval(Integer.parseInt(xxx.get(k)) - 300, Integer.parseInt(yyy.get(k)) - 300,
										300 * 2, 300 * 2);
			...
			if (true) {
				...
				if (tempARSUList.length > 0) {
					for (int l = 0; l < tempARSUList.length; l++) {
						if (highlightCommunication_)
							g2d.drawOval(tempARSUList[l].getX() - tempARSUList[l].getWifiRadius(),
									tempARSUList[l].getY() - tempARSUList[l].getWifiRadius(),
									tempARSUList[l].getWifiRadius() * 2, tempARSUList[l].getWifiRadius() * 2);
						// g2d.fillOval(tempARSUList[l].getX() - 250, tempARSUList[l].getY() -
						// 250,500,500);
						g2d.drawImage(TAImg_, tempARSUList[l].getX() - 2500, tempARSUList[l].getY() - 2500, 5000, 5000,
								null);
					}
				}
			}
			...
			while (tmpEventSpot != null) {
				...
				g2d.drawOval(tmpEventSpot.getX_() - tmpEventSpot.getRadius_(),
						tmpEventSpot.getY_() - tmpEventSpot.getRadius_(), tmpEventSpot.getRadius_() * 2,
						tmpEventSpot.getRadius_() * 2);
			...
			for (Cluster cluster : clusters) {
				...
				g2d.drawRect(cluster.getMinX_(), cluster.getMinY_(), (cluster.getMaxX_() - cluster.getMinX_()),
						(cluster.getMaxY_() - cluster.getMinY_()));
			}
			...
			if (showAllClusters) {
				for (Cluster cluster : clusters) {
					...
					for (int e = 0; e < cluster.getxCoords_().size(); e++) {
						...
						g2d.drawOval(cluster.getxCoords_().get(e) - 500, cluster.getyCoords_().get(e) - 500, 1000,
								1000);
					}
				}
				...
				for (int e = 0; e < displayCluster_.getxCoords_().size(); e++) {
					...
					g2d.drawOval(displayCluster_.getxCoords_().get(e) - 500, displayCluster_.getyCoords_().get(e) - 500,
							1000, 1000);
				}
			}
			...
			if (grid != null) {
				...
				for (int a = 0; a < grid.length; a++) {
					for (int b = 0; b < grid[0].length; b++) {
						...
						// g2d.setPaint(new Color((int)(255 * (((double)grid[a][b] - minEVA)) /
						// maxMinusmin), 255 - (int)(255 * (((double)grid[a][b] - minEVA)) /
						// maxMinusmin), 0));
						...
						g2d.drawRect(a * gridSize_ - (gridSize_ - strokeSize) / 2,
								b * gridSize_ - (gridSize_ - strokeSize) / 2, gridSize_ - strokeSize,
								gridSize_ - strokeSize);
	 ...
	 * 
	 * @param direction
	 *            <code>u</code> to pan up, <code>d</code> to pan down,
	 *            <code>l</code> to pan left, <code>r</code> to pan right
	 */
	 ...
	 * 
	 * @param x
	 *            the value for how far to pan in x direction (in map scale!)
	 * @param y
	 *            the value for how far to pan in y direction (in map scale!)
	 */
	...
	public synchronized void setMapZoom(double zoom) {
		if (zoom > 0.0000045 && zoom < 0.5) { // limit zooming by mousewheel into range from 100km to 1m
			...
			// (link last visited: 21.20.2008)
			// and the comments there. getScaledImage() is a little bit better quality but
			// is deprecated according to this as it's slow.
			...
			if (size < blockingImage_.getWidth() / 2) { // new image is smaller than the original
				...
				scaledBlockingImage_ = gc.createCompatibleImage(blockingImage_.getWidth(), blockingImage_.getHeight(),
						Transparency.TRANSLUCENT);
				new ConvolveOp(blurKernel).filter(blockingImage_, scaledBlockingImage_);

				scaledSlipperyImage_ = gc.createCompatibleImage(slipperyImage_.getWidth(), slipperyImage_.getHeight(),
						Transparency.TRANSLUCENT);
	...
	/**
	 * Updates various internal parameters after changes through panning or
	 * zooming.<br>
	 * The regions which shall be drawn are calculated here. They are a little bit
	 * larger than normally necessary in order to correctly display communication
	 * distance and mix zones. Otherwise, the distance of a vehicle, which is a
	 * little bit outside the currently viewable area wouldn't be drawn. This all
	 * induces a little bit of an unnecessary overdraw to the static objects but
	 * this should not matter a lot.
	 */
	public synchronized void updateParams() {
		if (map_.getReadyState() == true) {
			transform_.setToScale(zoom_, zoom_); // set the zoom
			transform_.translate(-middleX_ + (drawWidth_ / (zoom_ * 2)), -middleY_ + (drawHeight_ / (zoom_ * 2))); // pan
																													// and
																													// correct
																													// center
																													// of
			...
			// add a value on top to be able to correctly display communication distance and
			// mix radius
	...
	/**
	 * Schedules an update of the {@link DrawingArea}. Note that depending on if a
	 * simulation is running or not, the update might be performed at a later time!
	 * 
	 * @param fullRender
	 *            <code>true</code> if a full update including all static objects
	 *            should be done, else <code>false</code>
	 * @param forceRenderNow
	 *            <code>true</code> to force an immediate update regardless of
	 *            consistency considerations (should only be used by the
	 *            {@link vanetsim.simulation.SimulationMaster})
	 */
	 ...
	 * 
	 * @param running
	 *            <code>true</code> if a simulation is currently running,
	 *            <code>false</code> if it's suspended
	 */
	 ...
	 * 
	 * @param attackerVehicle
	 *            the attacker vehicle
	 */
	 ...
	 * 
	 * @param x
	 *            the new x coordinate for the center of the viewable area
	 * @param y
	 *            the new y coordinate for the center of the viewable area
	 */
	 ...
	 * 
	 * @param drawArea
	 *            the area on which this Renderer draws
	 */
	 ...
	 *            <code>true</code> if you want to highlight nodes, else
	 *            <code>false</code>
	 */
	 ...
	 * 
	 * @param highlightCommunication
	 *            <code>true</code> if you want to show the circles, else
	 *            <code>false</code>
	 */
	 ...
	 * 
	 * @param displayVehicleIDs
	 *            <code>true</code> if you want to show the IDs, else
	 *            <code>false</code>
	 */
	 ...
	 * 
	 * @param showAllBlockings
	 *            <code>true</code> if you want to show all blockings, else
	 *            <code>false</code>
	 */
	 ...
	 * 
	 * @param showBeaconMonitorZone
	 *            <code>true</code> if you want to display the monitored beacon
	 *            zones, else <code>false</code>
	 */
	...
	/**
	 * Sets the values for the monitored beacon zone. A rectangular bounding box
	 * within the specified coordinates is monitored if
	 * {@link #setShowBeaconMonitorZone(boolean)} is set to <code>true</code>.
	 * 
	 * @param beaconMonitorMinX
	 *            the minimum x coordinate
	 * @param beaconMonitorMaxX
	 *            the maximum x coordinate
	 * @param beaconMonitorMinY
	 *            the minimum y coordinate
	 * @param beaconMonitorMaxY
	 *            the maximum y coordinate
	 */
	public void setMonitoredBeaconZoneVariables(int beaconMonitorMinX, int beaconMonitorMaxX, int beaconMonitorMinY,
	 ...
	 * 
	 * @param showVehicles
	 *            <code>true</code> if you want to show all vehicles, else
	 *            <code>false</code>
	 */
	 ...
	 * 
	 * @param showMixZones
	 *            <code>true</code> if you want to show all mix zones, else
	 *            <code>false</code>
	 */
	 ...
	 * 
	 * @param autoAddMixZones
	 *            <code>true</code> if you want to add mix zones to all street
	 *            corners automatically else <code>false</code>
	 */
	 ...
	 * 
	 * @param showRSUs
	 *            <code>true</code> if you want to show all RSUs, else
	 *            <code>false</code>
	 */
	...
	/**
	 * @param markedJunction_
	 *            the markedJunction_ to set
	 */
